...
The environment is loaded on user log-in from `/etc/profile.d`, but if
...
Note that the `textFile` function returns an [`RDD`][spark-doc-rdd]
...
> Do not leave an interactive shell open when not using it. Interactive
